...
def getYesNo(msg):
    ...
    # Mock input during testing
    if 'pytest' in sys.modules:  # Verifica si estamos en un entorno de pruebas
        return True  # Simula que el usuario siempre responde "Yes"
    answer = ask(msg + " (Y/N)")
    answer = answer.upper()
    if answer in ['YES', 'Y']:
        return True
...
def scan(device_name=None):
    ...
    while True:
        ...
        if 'pytest' not in sys.modules:  # Solo solicita entrada si no estamos en un entorno de pruebas
            ask("remove %s, then press ENTER" % device_name)  # Pregunta solo en ejecución normal
        message("scanning...")
        time.sleep(DRIVER_UNLOAD_TIME)  # para permitir que el controlador se descargue
        before = ports.scan()
        beforec = len(before)
        ...
        if 'pytest' not in sys.modules:  # Solo solicita entrada si no estamos en un entorno de pruebas
            ask("plug in %s, then press ENTER" % device_name)  # Pregunta solo en ejecución normal
        message("scanning...")
        time.sleep(DRIVER_LOAD_TIME)  # para permitir que el controlador se cargue
        after = ports.scan()
        afterc = len(after)
        ...
        if len(added) == 0:
            ...
            while True:
                ...
                if 'pytest' in sys.modules:
                    return added[0]  # Selecciona automáticamente el primer dispositivo durante las pruebas
            ...
            # Solo 1 nuevo, selecciona automáticamente
